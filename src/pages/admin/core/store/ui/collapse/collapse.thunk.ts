...
export const saveCollapseState = createAsyncThunk(
  'collapse/saveState',
  async (params: { userId: string; collapseStates: Record<string, boolean> }, { rejectWithValue }) => {
    try {
      const result = await collapseApi.saveCollapseState(params.userId, params.collapseStates);
      return result;
    } catch (error) {

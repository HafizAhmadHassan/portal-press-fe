...
interface StatCardProps {
  number: number;
  label: string;
  variant: 'statCard--total' | 'statCard--active' | 'statCard--inactive' | 'statCard--blocked' | 'statCard--ready';
  className?: string;
}
...
export const DevicesSummaryBar: React.FC<DevicesSummaryBarProps> = ({
  ...
  const blockedDevices = devices.filter((d) => d.statusMachineBlocked === true).length;
  const readyDevices = devices.filter((d) => d.statusReadyD75_3_7 === true).length;
    ...
    <div className={`${styles.devicesSummary} ${className}`}>
      ...
      <div className={styles.mobileLayout}>
        ...
        <div className={styles.mobileStats}>
          <StatCard number={activeDevices} label="Attivi" variant="statCard--active" />
          <StatCard number={inactiveDevices} label="Inattivi" variant="statCard--inactive" />
          <StatCard number={blockedDevices} label="Bloccati" variant="statCard--blocked" />
          <StatCard number={readyDevices} label="Pronti" variant="statCard--ready" />
        </div>
      </div>
      ...
      <div className={styles.desktopLayout}>
        <div className={styles.statsSection}>
          <StatCard number={totalDevices} label="Totali" variant="statCard--total" />

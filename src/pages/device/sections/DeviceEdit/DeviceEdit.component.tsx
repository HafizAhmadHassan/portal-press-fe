...
// ⚠️ Adatta questi import ai tuoi endpoint RTK Query reali:
import {
...
export default function DeviceEditPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDevice({ id: device.id, ...payload }).unwrap();
      setDirty(false);
      // torna all'overview del device
      navigate(`/device/${device.id}`);
    } catch (e) {
      console.error("PAGE_EDIT_DEVICE - Errore nel salvataggio:", e);
      ...
      <section className={styles.page}>
        <div className={styles.pageHeader}>
          <button className={styles.backBtn} onClick={() => navigate("/admin")}>
            <ArrowLeft size={18} />
            <span>Torna all’area Admin</span>
          </button>
        </div>
        <div className={styles.emptyState}>
          Nessun device selezionato. Apri un device dalla lista.
        </div>
      </section>
    );
  }

  if (isError || !device || !formData) {
    return (
      <section className={styles.page}>
        <div className={styles.pageHeader}>
        ...
        <div className={styles.errorState}>
          <p>Impossibile caricare il dispositivo.</p>
  ...
  return (
    <section className={styles.page}>
      {/* Contenuto (stesso layout della modale) */}

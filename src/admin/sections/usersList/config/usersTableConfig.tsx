...
export const createUsersTableConfig = ({
  ...
  return {
    columns: [
      {
        ...
        render: (_: any, user: User) => (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Avatar user={user} />
            <div>
              <div style={{ fontWeight: 600, fontSize: '14px', color: tPrimary }}>
                {user.full_name ||
                  `${user.firstName || ''} ${user.lastName || ''}`.trim() ||
                  user.username}
              </div>
              {user.email && (
                <div style={{ fontSize: '12px', color: tSecondary }}>{user.email}</div>
              )}
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        sortable: true,
        render: (_: any, user: User) => {
          if (!user.date_joined) return <span style={{ color: tSecondary }}>N/A</span>;
    ...
    emptyMessage: 'Nessun utente trovato. Prova a modificare i filtri di ricerca.',

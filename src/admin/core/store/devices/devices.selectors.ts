...
export const selectAllDevicesLoading = (state: RootState) => state.devices.isLoadingAll;
export const selectAllDevicesError = (state: RootState) => state.devices.allDevicesError;
...
export const selectIndifferenziatedDevices = createSelector([selectDevices], (devices) =>
...
export const selectAllPlasticDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.waste === 'Plastica')
);

export const selectAllDryDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.waste === 'Secco')
);

export const selectAllWetDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.waste === 'Umido')
);

export const selectAllGlassDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.waste === 'Vetro')
);

export const selectAllIndifferenziatedDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.waste === 'Indifferenziato')
);

export const selectAllCartaDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.waste === 'Carta')
);

export const selectAllVplDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.waste === 'vpl')
);
...
export const selectInactiveDevices = createSelector([selectDevices], (devices) =>
  devices.filter((device: Device) => device.status === 0)
);

export const selectBlockedDevices = createSelector([selectDevices], (devices) =>
  devices.filter((device: Device) => device.statusMachineBlocked === true)
);

export const selectReadyDevices = createSelector([selectDevices], (devices) =>
  devices.filter((device: Device) => device.statusReadyD75_3_7 === true)
);

// NUOVO: Selettori per TUTTI i devices per status (per la mappa)
export const selectAllActiveDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.status === 1)
);

export const selectAllInactiveDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.status === 0)
);

export const selectAllBlockedDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.statusMachineBlocked === true)
);

export const selectAllReadyDevices = createSelector([selectAllDevices], (allDevices) =>
  allDevices.filter((device: Device) => device.statusReadyD75_3_7 === true)
);
...
export const selectDevicesCount = createSelector([selectDevices], (devices) => {
  ...
  return {
    ...
    blocked: devices.filter((d) => d.statusMachineBlocked === true).length,
    ready: devices.filter((d) => d.statusReadyD75_3_7 === true).length,
    ...
    indifferenziato: devices.filter((d) => d.waste === 'Indifferenziato').length,
...
export const selectAllDevicesCount = createSelector([selectAllDevices], (allDevices) => {
...
export const selectDevicesByCity = createSelector([selectDevices], (devices) => {
...
export const selectAllDevicesByCity = createSelector([selectAllDevices], (allDevices) => {
  const devicesByCity: Record<string, Device[]> = {};
...
export const selectDevicesByCustomer = createSelector([selectDevices], (devices) => {
...
export const selectAllDevicesByCustomer = createSelector([selectAllDevices], (allDevices) => {
  const devicesByCustomer: Record<string, Device[]> = {};
  allDevices.forEach((device: Device) => {
    if (device.customer) {
      if (!devicesByCustomer[device.customer]) {
...
export const selectAllDevicesWithGPS = createSelector([selectAllDevices], (allDevices) =>

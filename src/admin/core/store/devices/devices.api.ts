  ...
  endpoints: (builder) => ({
    ...
    createDevice: builder.mutation<Device, CreateDeviceRequest>({
      query: (body) => {
        ...
        console.log('RTK Query - Body keys:', body ? Object.keys(body) : 'NO KEYS');
    ...
    deleteDevice: builder.mutation<{ success: boolean; message: string }, string>({
      query: (id) => {
      ...
      async onQueryStarted(id, { dispatch, queryFulfilled }) {
        ...
        dispatch(
          apiSlice.util.updateQueryData('getDevices', {}, (draft: any) => {
            if (draft.data) {
              const deviceIndex = draft.data.findIndex((device: Device) => device.id === id);
              if (deviceIndex !== -1) {
        ...
        dispatch(
          apiSlice.util.updateQueryData('getAllDevices', undefined, (draft: Device[]) => {
            const deviceIndex = draft.findIndex((device: Device) => device.id === id);
            if (deviceIndex !== -1) {
    ...
    toggleDeviceStatus: builder.mutation<Device, { id: string; status: number }>({
      query: ({ id, status }) => {
    ...
    toggleDeviceBlock: builder.mutation<Device, { id: string; blocked: boolean }>({
      query: ({ id, blocked }) => {
    ...
    bulkActions: builder.mutation<{ message: string; affectedCount: number }, BulkActionRequest>({
      query: (body) => ({
        url: 'devices/bulk/',
        method: 'POST',
        body,
